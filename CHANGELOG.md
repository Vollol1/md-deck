...
helper generated by `setup.sh` will not change without a major version bump.
...
- `scripts/run-deck.sh.template` — the helper that `setup.sh` copies to the
  wrapper root as `run-deck.sh`; it now auto-detects the engine in `decks/` or
  `../md-deck/`, with a `DECK_ENGINE_DIR` override for other layouts
